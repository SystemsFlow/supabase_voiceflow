...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: connection, error: connError } = await supabase
      .from('connections')
      .select('access_token, refresh_token, token_expires_at, database_id, schema_snapshot')
      .eq('user_id', user_id)
    ...
    let { access_token, refresh_token, token_expires_at, database_id, schema_snapshot } = connection

    // Refresh token se scaduto o in scadenza nei prossimi 5 minuti
    ...
    if (isExpired && refresh_token) {
      ...
      if (!refreshResponse.ok) {
        throw new Error('Sessione Airtable scaduta — riconnetti Airtable dalla dashboard')
      }
